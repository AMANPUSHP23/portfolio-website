    ...
    >
      <Card className={`relative overflow-hidden h-full flex flex-col group border-2 rounded-3xl bg-white/10 dark:bg-background/80 backdrop-blur-xl shadow-2xl transition-all duration-300 ${featured ? 'border-primary/70 shadow-primary/40' : 'border-border hover:border-primary/60 hover:shadow-2xl'}`}>
        ...
        <CardHeader className="p-4">
          <CardTitle className="text-base md:text-lg font-bold text-white/90 relative">
            {title}
            {/* Accent underline */}
        ...
        <CardContent className="flex-grow p-4 pt-0">
          <div className="relative mb-2 h-16 group/scroll">
            <p className="text-xs md:text-sm text-muted-foreground h-full overflow-y-auto pr-2 custom-scrollbar">
              {description}
            </p>
            {/* Gradient fade at bottom to indicate more content */}
            <div className="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-card to-transparent pointer-events-none opacity-0 group-hover/scroll:opacity-100 transition-opacity duration-300"></div>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 text-xs bg-primary/10 text-primary rounded-full font-semibold shadow-sm transition-all duration-300 hover:bg-primary/20 hover:shadow-[0_0_8px_2px_rgba(236,72,153,0.25)]"
              >
                {tag}
              </span>
            ))}
        ...
        <CardFooter className="flex justify-end space-x-2 mt-auto pt-3 px-4 pb-4 border-t border-border">
          {repoUrl && repoUrl !== "#" && (
